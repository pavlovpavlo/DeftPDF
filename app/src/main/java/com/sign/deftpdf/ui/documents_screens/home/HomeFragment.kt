...
class HomeFragment : BaseFragment(R.layout.fragment_home), DocumentsView, DocumentsAdapter.OnDocumentClickListener {
    ...
    private fun initListeners() {
        with(binding) {
            ...
            adapter.listener = this@HomeFragment
    ...
    private fun showSortDialog() {
        ...
        reset.setOnClickListener {
            DeftApp.sortTypeHome = Util.SORT_DESC
            list.clear()
            sendRequest()
            dismissPopup()
        }
        sortGroup.check(
                if (DeftApp.sortTypeHome == Util.SORT_DESC)
                    R.id.latest_sort
                else
                    R.id.oldest_sort)

        sortGroup.setOnCheckedChangeListener { group, checkedId ->
            DeftApp.sortTypeHome = if (checkedId == R.id.latest_sort)
                Util.SORT_DESC
            else
                Util.SORT_ASC
            list.clear()
    ...
    private fun showFilterDialog() {
        ...
        filterGroup.check(
                if (DeftApp.filterTypeHome == Util.FILTER_WEEK)
                    R.id.week_filter
                else
                    R.id.month_filter)

        filterGroup.setOnCheckedChangeListener { group, checkedId ->
            DeftApp.filterTypeHome = if (checkedId == R.id.week_filter)
                Util.FILTER_WEEK
            else
                Util.FILTER_MONTH
            list.clear()
